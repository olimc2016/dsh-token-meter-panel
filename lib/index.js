...
export function apply(ctx, config = {}) {
  ...
  function summary({ force = false } = {}) {
    ...
    const task = (async () => {
      ...
      // 首屏不阻塞：官方数据可能还没到（需要读浏览器登录态 + 3 次 HTTP）。
      // 有缓存就直接用缓存，没有就先标 loading，后台补齐，下一次轮询/客户端快速重试就拿到。
      value.official = official.payload ?? { ok: false, reason: 'loading' };
      if (!official.payload || Date.now() - official.at >= USAGE_TTL_MS) {
        fetchOfficial().catch(() => { /* 失败会在下一次轮询体现 */ });
      }
