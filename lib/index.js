...
/* 浏览器登录态读取器用**动态导入**：任何加载期失败都不能拖垮插件本身，更不能让 DSH 起不来
 * （0.5.0 的教训：静态命名导入 node:zlib 的 zstdDecompressSync，老 Node 上直接链接期报错） */
let browserTokenReader;
/** token 内存缓存：读一次能管很久（登录态几小时才变），避免每 5 分钟重拷一遍 leveldb */
let browserTokenCache = { value: null, at: 0 };
let browserTokenPending = null;
const BROWSER_TOKEN_TTL_MS = 30 * 60 * 1000;
/** 读登录态的单次预算：超时先按「未连接」处理，后台读到了下次自动用上，绝不拖住首屏 */
const BROWSER_TOKEN_BUDGET_MS = 3000;

async function readTokenFromBrowser() {
  if (browserTokenCache.value && Date.now() - browserTokenCache.at < BROWSER_TOKEN_TTL_MS) {
    return browserTokenCache.value;
  }
  if (browserTokenReader === undefined) {
    try {
      browserTokenReader = (await import('../src/chrome-storage.mjs')).readDeepseekPlatformToken ?? null;
    } catch {
      browserTokenReader = null; // 读取器不可用就静默降级（面板会显示「未连接」），绝不抛给上层
    }
  }
  if (!browserTokenReader) return null;
  // 同一个读取任务并发共用；读到了就进缓存（哪怕是这次超时之后才读到）
  if (!browserTokenPending) {
    browserTokenPending = Promise.resolve()
      .then(() => browserTokenReader())
      .then((token) => {
        if (token) browserTokenCache = { value: token, at: Date.now() };
        return token ?? null;
      })
      .catch(() => null)
      .finally(() => { browserTokenPending = null; });
  }
  const budget = new Promise((resolve) => setTimeout(() => resolve(undefined), BROWSER_TOKEN_BUDGET_MS));
  const got = await Promise.race([browserTokenPending, budget]);
  return got ?? browserTokenCache.value ?? null;
}
...
/* ------------------------------------------------------------------ *
 * 版本与更新：面板底部显示版本号 + 开源协议，并提示是否有新版本。
 *   只查 GitHub Releases（公开接口），不下载、不自动安装；
 *   更新动作交给用户一条命令（复制按钮），因为改 profile 需要 pnpm + 重启。
 * ------------------------------------------------------------------ */
const PLUGIN_REPO = 'olimc2016/dsh-token-meter-panel';
const PLUGIN_LICENSE = 'MIT';
const REPO_URL = `https://github.com/${PLUGIN_REPO}`;
const UPDATE_TTL_MS = 6 * 3600 * 1000;
/** 官方取数的硬超时：超过就落成明确失败，绝不让面板停在「正在获取」 */
const OFFICIAL_FETCH_TIMEOUT_MS = 25000;
...
/** 查最新 Release（失败就静默返回 null，不影响面板） */
async function fetchLatestRelease() {
  const res = await fetch(`https://api.github.com/repos/${PLUGIN_REPO}/releases/latest`, {
    headers: { accept: 'application/vnd.github+json', 'user-agent': 'dsh-token-meter-panel' },
    signal: AbortSignal.timeout(10000),
  });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const body = await res.json();
  return {
    tag: String(body?.tag_name ?? '').replace(/^v/, ''),
...
const DEFAULTS = {
  ...
  skin: 'glass',
  /** 后台静默更新：发现新版本后自动升级 profile，下次重启 DSH 即生效 */
  autoUpdate: true,
  refreshSeconds: 60,
  officialOnly: true,
  offPeak: {
...
export async function loadConfigSchema() {
  try {
    ...
    return z.object({
      ...
      autoUpdate: z.boolean().default(DEFAULTS.autoUpdate)
        .description('后台静默更新：发现新版本自动升级 profile，下次重启 DSH 生效（会执行 dsh plugin add 命令）'),
...
export function apply(ctx, config = {}) {
  ...
  function summary({ force = false } = {}) {
    ...
    const task = (async () => {
      ...
      const value = aggregate(sessionsRoot, {
        ...
        // 硬时间预算：会话日志很大时（工作电脑曾要 70 秒）最多扫这么久，先用已扫到的部分出数
        budgetMs: Number(cfg.scanBudgetMs) > 0 ? Number(cfg.scanBudgetMs) : 8000,
      });
      ...
      // 官方数据：**在同一个响应里带回**（现在实测取数只要 ~325ms，等得起）。
      // 之前做成"后台补齐 + 客户端轮询"会因为缓存时序对不上，面板长时间停在「正在获取」。
      value.official = await officialForSummary();
      // 官方用量可用时，天/时维度直接采用官方数据（与平台用量页同源）；按会话明细仍来自本地日志
      if (value.official?.ok && value.official.days) {
        const o = value.official;
        const localDays = value.days; // 本地聚合结果：含「峰/谷分档」的 token 与金额
        const officialDays = {};
        for (const [date, d] of Object.entries(o.days)) {
          ...
          officialDays[date] = {
            ...
            // 中栏要「峰/谷分开算再汇总」：分档数据来自本地日志（官方接口没有这个维度）
            tier: localDays?.[date]?.tier,
          };
        }
      ...
      // 版本 / 协议 / 更新提示（不阻塞首屏，缓存 6 小时，后台刷新）
      value.plugin = {
        name,
        version: PKG.version,
        license: PLUGIN_LICENSE,
        repo: REPO_URL,
        update: updateInfo.payload
          ? {
            ...updateInfo.payload,
            running: autoUpdateState.running,
            applied: autoUpdateState.done === updateInfo.payload.latest,
            error: autoUpdateState.error,
          }
          : null,
      };
      if (!updateInfo.payload || Date.now() - updateInfo.at >= UPDATE_TTL_MS) {
        checkUpdate()
          .then((info) => {
            // 后台静默更新：插件跑满 60 秒后才动手（避免和启动抢资源），且只做一次
            if (info?.hasUpdate && settings().autoUpdate !== false) {
              setTimeout(() => {
                if (Date.now() - mountedAt >= 60000) runBackgroundUpdate(info.latest);
              }, 60000);
            }
          })
          .catch(() => { /* 查不到就不提示 */ });
  ...
  let official = { at: 0, payload: null, inflight: null };

  /**
   * 取官方数据：5 分钟缓存；并发请求共用同一次取数（inflight）；
   * 套 25 秒硬超时护栏，保证**一定**返回终态，绝不挂起。
   */
  async function officialForSummary() {
    const now = Date.now();
    if (official.payload && now - official.at < USAGE_TTL_MS) return official.payload;
    if (!official.inflight) {
      const started = Date.now();
      logger?.info?.('official usage: fetch start');
      const guard = new Promise((resolve) => {
        setTimeout(() => resolve({ ok: false, reason: 'timeout', error: `取数超过 ${OFFICIAL_FETCH_TIMEOUT_MS / 1000} 秒` }), OFFICIAL_FETCH_TIMEOUT_MS);
      });
      official.inflight = Promise.race([fetchOfficial(), guard])
        .then((payload) => {
          const settled = payload ?? { ok: false, reason: 'no-result' };
          official = { at: Date.now(), payload: settled, inflight: null };
          logger?.info?.(`official usage: ${settled.ok ? 'ok' : `fail(${settled.reason})`} in ${Date.now() - started}ms`);
          return settled;
        })
        .catch((error) => {
          const settled = { ok: false, reason: 'request-failed', error: `${error?.message ?? error}` };
          official = { at: Date.now(), payload: settled, inflight: null };
          logger?.warn?.(`official usage failed: ${error?.message ?? error}`);
          return settled;
        });
    }
    return official.inflight;
  }
  ...
  async function fetchOfficial({ force = false } = {}) {
    ...
    if (!token) {
      ...
      try {
        token = await readTokenFromBrowser();
  ...
  // ---- 后台静默更新 ----
  // 只允许一条白名单命令：dsh plugin --profile <本 profile> add github:olimc2016/dsh-token-meter-panel#v<版本>
  // 不接受任何外部输入拼接；升级只改 profile 里的依赖，**下次重启 DSH 才生效**（插件是启动时加载的）。
  // 绝不在启动阶段执行：只在插件运行满 60 秒后、由「发现新版本」触发，且每个版本只做一次。
  const mountedAt = Date.now();
  let autoUpdateState = { running: false, done: null, error: null };
  function runBackgroundUpdate(tag) {
    if (!/^\d+\.\d+\.\d+$/.test(tag)) return; // 版本号必须形如 1.2.3，杜绝命令注入
    if (autoUpdateState.running || autoUpdateState.done === tag) return;
    autoUpdateState = { running: true, done: null, error: null };
    const profile = process.env.DSH_PROFILE || config.profile || 'desktop';
    const spec = `github:${PLUGIN_REPO}#v${tag}`;
    logger?.info?.(`auto update: start ${spec} (profile=${profile})`);
    let child;
    try {
      child = spawn('dsh', ['plugin', '--profile', profile, 'add', spec], {
        stdio: 'ignore', shell: process.platform === 'win32', windowsHide: true,
      });
    } catch (error) {
      autoUpdateState = { running: false, done: null, error: `${error?.message ?? error}` };
      logger?.warn?.(`auto update spawn failed: ${error?.message ?? error}`);
      return;
    }
    child.on('error', (error) => {
      autoUpdateState = { running: false, done: null, error: `${error?.message ?? error}` };
      logger?.warn?.(`auto update error: ${error?.message ?? error}`);
    });
    child.on('exit', (code) => {
      if (code === 0) {
        autoUpdateState = { running: false, done: tag, error: null };
        logger?.info?.(`auto update: ok → v${tag}（重启 DSH 后生效）`);
      } else {
        autoUpdateState = { running: false, done: null, error: `dsh 退出码 ${code}（可能缺 pnpm）` };
        logger?.warn?.(`auto update failed: exit ${code}`);
      }
    });
  }

  // ---- 版本更新检查（只查 Release，不下载不安装；失败静默） ----
  let updateInfo = { at: 0, payload: null };
  async function checkUpdate({ force = false } = {}) {
    const now = Date.now();
    if (!force && updateInfo.payload && now - updateInfo.at < UPDATE_TTL_MS) return updateInfo.payload;
    try {
      const latest = await fetchLatestRelease();
      const payload = {
        current: PKG.version,
        latest: latest.tag,
        hasUpdate: Boolean(latest.tag) && compareVersion(latest.tag, PKG.version) > 0,
        url: latest.url,
        checkedAt: now,
        // 用户可以直接复制这条命令升级（改 profile 需要 pnpm + 重启，所以不替用户执行）
        command: `dsh plugin --profile desktop add github:${PLUGIN_REPO}#v${latest.tag}`,
      };
      updateInfo = { at: now, payload };
      return payload;
    } catch (error) {
      logger?.warn?.(`update check failed: ${error?.message ?? error}`);
      updateInfo = { at: now, payload: updateInfo.payload };
  ...
  ctx.effect(() => {
    ...
    // 注意：这里**绝对不能**在启动阶段预热 summary ——
    // aggregate() 是同步 CPU 重活（解压 + 扫描全部会话日志），在启动窗口里跑会堵住事件循环，
    // 导致 DSH 的「启动插件服务」调用超时并进入恢复模式（0.4.1~0.5.1 踩过）。
    // 首屏速度改由客户端负责：先渲染 + 读秒，再等 /summary 返回，必要时 1.2s 快速重试。
    return () => {
