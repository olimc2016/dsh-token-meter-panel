...
${t("hitInput")} ${fmt(entry.hit)} \xB7 ${t("missInput")} ${fmt(entry.miss)} \xB7 ${t("output")} ${fmt(entry.out)}` : `${label} \xB7 ${t("zeroDays")}`;
      ...
      import_react.default.createElement("div", { key: "split", style: S.card }, [
        import_react.default.createElement("div", {
          ...
          import_react.default.createElement(
            "span",
            { key: "dbg", style: { color: C.warn, fontSize: 10, marginLeft: 10, fontVariantNumeric: "tabular-nums" } },
            `[sel=${selDay ?? "null"} day=${selDay && days[selDay] ? "y" : "n"} tier=${SD.tier ? "y" : "n"} hit=${Math.round(SD.hit ?? -1)} miss=${Math.round(SD.miss ?? -1)} out=${Math.round(SD.out ?? -1)} cost=${sHit.toFixed(3)}/${sMiss.toFixed(3)}/${sOut.toFixed(3)} total=${splitTotal.toFixed(3)}]`
          )
        ]),
        import_react.default.createElement(
          "div",
