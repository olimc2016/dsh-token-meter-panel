...
function TokenMeterPanel(props) {
  ...
  const costOf = (which) => {
    ...
    const at2 = (tier) => ((SD.tier[tier] ?? {})[which] ?? 0) / 1e6 * rateOf(cfgRates?.[tier]?.["deepseek-flash"], which);
    return at2("peak") + at2("offPeak");
  };
  const sHit = costOf("hit");
  const sMiss = costOf("miss");
  const sOut = costOf("out");
  const tokOf = (which) => SD.tier ? (SD.tier.peak?.[which] ?? 0) + (SD.tier.offPeak?.[which] ?? 0) : SD[which] ?? 0;
...
${t("hitInput")} ${fmt(entry.hit)} \xB7 ${t("missInput")} ${fmt(entry.miss)} \xB7 ${t("output")} ${fmt(entry.out)}` : `${label} \xB7 ${t("zeroDays")}`;
        ...
        import_react.default.createElement("div", { key: "hr", style: { marginTop: 14, paddingTop: 12, borderTop: `1px solid ${C.border1}` } }, [
          ...
          import_react.default.createElement("div", {
            ...
            ...peakHoursToday[9] ? [9, 12, 14, 18].map((h) => import_react.default.createElement("div", {
              key: `pk${h}`,
              style: {
                position: "absolute",
                top: 0,
          ...
          }, Array.from({ length: 24 }, (_, h) => import_react.default.createElement("span", {
            key: h,
      ...
      import_react.default.createElement("div", { key: "split", style: S.card }, [
        import_react.default.createElement("div", {
          ...
          import_react.default.createElement(
            "span",
            { key: "basis", style: { fontSize: 10.5, color: C.label3, border: `1px solid ${C.border}`, borderRadius: 999, padding: "1px 7px" } },
            t("byAmount")
          ),
        ...
        import_react.default.createElement(
          ...
          ].map(([label, color, cost, tokens]) => import_react.default.createElement("div", {
            key: label,
            style: { display: "grid", gridTemplateColumns: "150px 1fr 84px", gap: 10, alignItems: "center", fontSize: 12 }
          }, [
            import_react.default.createElement("div", { key: "n", style: { display: "flex", flexDirection: "column", gap: 1, color: C.label2, whiteSpace: "nowrap" } }, [
              import_react.default.createElement(
                "span",
                { key: "l", style: { display: "flex", alignItems: "center", gap: 6 } },
                [import_react.default.createElement("i", { key: "i", style: swatch(color) }), label]
              ),
              import_react.default.createElement(
                "span",
                { key: "k", style: { fontSize: 10, color: C.label3, marginLeft: 14, fontVariantNumeric: "tabular-nums" } },
                `${fmtM(tokens)} tokens`
              )
            ]),
            import_react.default.createElement("div", {
              ...
              return [
                ...
                import_react.default.createElement("span", {
                  key: "p",
                  style: wide ? { position: "absolute", right: 6, top: -3, fontSize: 11, fontWeight: 700, color: "#04070d", fontVariantNumeric: "tabular-nums" } : { position: "absolute", left: `calc(${p.toFixed(1)}% + 6px)`, top: -3, fontSize: 11, fontWeight: 600, color: C.label2, whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums" }
                }, `${p.toFixed(0)}%`)
              ];
            })()),
            import_react.default.createElement("div", {
